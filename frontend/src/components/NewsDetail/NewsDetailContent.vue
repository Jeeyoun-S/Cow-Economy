<template>
  <v-sheet class="pa-7 d-flex flex-column">
    ...
    <div class="py-2 d-flex align-center">
      ...
      <v-btn class="ml-auto" color="grey darken-1" small outlined rounded
        >원본 보기</v-btn
      >
    </div>
    ...
    <v-snackbar v-model="memoBtn" :multi-line="true">
      ...
      <template v-slot:action="{ attrs }">
        <v-btn color="red" text v-bind="attrs" @click="changeMemoBtn()">
          Close
        </v-btn>
        <v-btn color="red" text v-bind="attrs" @click="addMemoReference()">
          추가
        </v-btn>
      </template>
    </v-snackbar>
...
export default {
  ...
  computed: {
    getMemoBtn() {
      return this.$store.getters.getMemoBtn;
    },
  },
  methods: {
    changeMemoBtn() {
      this.$store.dispatch("changeMemoBtn");
    },
    addMemoReference() {
      document.getElementById("memo").scrollIntoView(true);
      this.$store.dispatch("getSelectionText");
    },
  },
  watch: {
    getMemoBtn() {
      this.memoBtn = this.$store.getters.getMemoBtn;
    },
  },
  created() {
